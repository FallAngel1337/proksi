...
impl Server {
    ...
    pub async fn start(self) -> io::Result<()> {
        ...
        loop {
            ...
            if Self::handle_establish(&mut delivery, methods)
                .await
                .is_err()
            {
                break Ok(());
    ...
    async fn handle_establish(
        ...
        if methods.iter().any(|x| estbl_req.methods().contains(x)) {
            if let Some(&val) = methods.iter().max_by_key(|&&x| x) {
                delivery.send(EstablishResponse::new(val)).await?;
            }
        } else {
            delivery
                .send(EstablishResponse::new(method::NO_ACCEPTABLE_METHODS))
                .await?;
